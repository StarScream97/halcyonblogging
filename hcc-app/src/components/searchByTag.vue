      ...
      <div id="posts" v-for="post in allPosts.slice().reverse()" v-bind:key=post._id>
        <div class="post my-5">
          <div class="authorInfo">
               <!-- <router-link to="/user"><img :src="post.author.profileImg" class="circle" alt=""></router-link> -->
          ...
          <div class="blogContents px-3 py-4">
             <router-link :to="{ name: 'singlePost', params: { title:post._id}}"><h3 class="postTitle">{{post.title}}</h3></router-link> 
              ...
              <div class="tags" v-for="tag in post.tags" v-bind:key=tag.index v-on:click.prevent="lyricPost">
...
export default {
  ...
  methods: {
    lyricPost(){
      this.allPosts=[];
    axios.get(`/posts/tags/${this.$route.params.tag}`).then(res => {
